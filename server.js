...
// Лимит 50МБ (стабильно работает на Railway)
...
// Безопасная функция для выполнения SQL без падения сервера
...
async function initDB() {
  ...
  // 2. Добавляем колонки в существующие таблицы (на случай обновлений)
  const tables = ['users', 'messages', 'private_messages', 'rooms', 'room_messages'];
  const columns = [
    ['users', 'banned', 'BOOLEAN DEFAULT false'],
    ['users', 'muted_until', 'BIGINT DEFAULT 0'],
    ['users', 'role', "VARCHAR(20) DEFAULT 'user'"],
    ['users', 'token', "VARCHAR(200) DEFAULT NULL"],
    ['rooms', 'comments_enabled', "BOOLEAN DEFAULT true"],
    ['messages', 'file_data', 'TEXT'],
    ['messages', 'file_name', 'TEXT'],
    ['private_messages', 'file_data', 'TEXT'],
    ['private_messages', 'file_name', 'TEXT'],
    ['room_messages', 'file_data', 'TEXT'],
    ['room_messages', 'file_name', 'TEXT']
  ];

  for (let [tbl, col, def] of columns) {
    await safeQuery(`ALTER TABLE ${tbl} ADD COLUMN IF NOT EXISTS ${col} ${def}`);
  }
